...
def generate_gradcam_images(img_input,
                            model_to_use,
                            layer_name,
                            input_size=(32,32),
    ...
    """
        Utility to generate final Gradcam images
    """
    normalizer = Normalize(mean=mean, std=std)
    torch_img = torch.from_numpy(np.asarray(img_input).reshape((1,3,32,32)))

    normed_torch_img = normalizer(torch_img)
    model_to_use = model_to_use.to(torch.device("cpu"))
    model_to_use.eval()

    resnet_model_dict = dict(type='resnet', arch=model_to_use, layer_name=layer_name, input_size=input_size)
    resnet_gradcam = GradCAM(resnet_model_dict, True)
