...
from miniRekog.train.traintest import execute_model
...
def main():
    ...
    trainloader = dataloader.get_dataloader(trainset, config['batch_size'], shuffle=True, num_workers=2)
    ...
    #model_new = basemodelclass.CIFARModelDepthDilate#.to(device)
    model_new = ResNet18() #config['dropout'])
    #execute_model(model, hyperparameter_defaults, )
    final_model_path = execute_model(model_new, config, 
